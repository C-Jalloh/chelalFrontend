...
// Change to .cjs for CommonJS compatibility
const axios = require('axios');
const API_URL = 'http://localhost:8000/api/patients/';

async function createPatient(patient) {
  // You may need to update this token to a valid one for your backend
...
async function addTestPatients() {
  ...
  for (const student of students) {
    try {
      ...
      console.error('Failed to add:', student, e.response ? e.response.data : e);
    }
  }
}
