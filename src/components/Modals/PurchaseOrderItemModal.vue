<template>
  <div class="purchase-order-item-modal" @mousedown.self="$emit('close')">
    <div class="modal-content">
      <h3 class="modal-title">{{ isEditing ? 'Edit Purchase Order Item' : 'Add Purchase Order Item' }}</h3>
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          ...
          <input id="quantity" type="number" v-model.number="item.quantity" min="1" required />
        </div>
...
import { createPurchaseOrderItem, updatePurchaseOrderItem } from '../services/purchaseOrderItemService';
...
const item = ref(props.existingItem ? { ...props.existingItem } : { name: '', quantity: 1 });
const errorMessage = ref('');

watch(() => props.existingItem, (newVal) => {
  isEditing.value = !!newVal;
  item.value = newVal ? { ...newVal } : { name: '', quantity: 1 };
});
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEditing.value && item.value.id) {
      const updated = await updatePurchaseOrderItem(item.value.id, item.value, token);
      emit('itemUpdated', updated.data);
    } else {
      const created = await createPurchaseOrderItem(item.value, token);
